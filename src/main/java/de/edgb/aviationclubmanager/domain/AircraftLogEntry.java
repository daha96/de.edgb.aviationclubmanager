...
public class AircraftLogEntry {
	...
	private static Boolean collectiveLogEntryPossible(Flight previousFlight,
			Flight flight) {
		...
		// lands
		// at the same place.
		if (previousFlight.getFlightMode() != FlightMode.local
				|| previousFlight.getFlightMode() != FlightMode.local)
			return false;
		if (!previousFlight.getDepartureLocation().equals(
				previousFlight.getLandingLocation()))
			return false;
		if (!previousFlight.getLandingLocation().equals(
				flight.getDepartureLocation()))
			return false;
		if (!flight.getDepartureLocation().equals(flight.getLandingLocation()))
			return false;
	 ...
	 * @return
	 */
	private static AircraftLogEntry createAircraftLogEntry(
		...
		for (Flight f : flightList) {
			...
			if (entry.minOccupants == 0
					|| f.getNumPassengers() < entry.minOccupants)
				entry.minOccupants = f.getNumPassengers();
			if (entry.maxOccupants == 0
					|| f.getNumPassengers() > entry.maxOccupants)
	...
	public static List<AircraftLogEntry> createAircraftLogEntriesFromFlights(
			List<Flight> flightList, Aircraft airc) {
		...
		for (Flight flight : fl) {
			...
			if (previousFlight != null
					&& !collectiveLogEntryPossible(previousFlight, flight)) {
				// No further merging
	...
	public static List<AircraftLogEntry> createAircraftLogEntriesFromFlights(
		...
		for (Aircraft a : aircrafts) {
			resultList.addAll(createAircraftLogEntriesFromFlights(
					finishedFlightList, a));
		}

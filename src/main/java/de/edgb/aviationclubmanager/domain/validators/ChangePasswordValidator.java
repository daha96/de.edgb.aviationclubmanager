...
public class ChangePasswordValidator extends AbstractValidator {
	...
	protected void addExtraValidation(Object objectForm, Errors errors) {
		...
		String originalPassword = ((UserAccountDetails)SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUserAccount().getPassword();
		
		if (!originalPassword.equals(encoder.encodePassword(cpm.getOldPassword(), null)))
			errors.rejectValue("oldPassword", "de_edgb_aviationclubmanager_domain_changepasswordmodel_oldpassword_invalid");

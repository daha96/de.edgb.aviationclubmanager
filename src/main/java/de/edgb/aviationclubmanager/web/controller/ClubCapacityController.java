...
public class ClubCapacityController {
	...
	public String create(@Valid ClubCapacity clubCapacity,
		...
		return "redirect:/clubcapacitys/"
				+ encodeUrlPathSegment(clubCapacity.getId().toString(),
						httpServletRequest);
	}
	...
	public String list(
		...
		if (page != null || size != null) {
			...
			uiModel.addAttribute("clubcapacitys",
					ClubCapacity.findClubCapacityEntries(firstResult, sizeNo));
			float nrOfPages = (float) ClubCapacity.countClubCapacitys()
					/ sizeNo;
	...
	public String update(@Valid ClubCapacity clubCapacity,
			BindingResult bindingResult, Model uiModel,
			HttpServletRequest httpServletRequest) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, clubCapacity);

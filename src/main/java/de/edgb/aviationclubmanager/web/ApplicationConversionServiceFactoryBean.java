	...
	private Converter<BigDecimal, String> createBigDecimalConverter() {
		return new Converter<BigDecimal, String>() {
			...
			public String convert(BigDecimal value) {
				String output = value.toString();
				try {
					output = NumberFormat.getInstance(
							LocaleContextHolder.getLocale()).format(value);// applicationContext.getMessage(value.toString(),
																			// null,
																			// LocaleContextHolder.getLocale());
				} catch (NoSuchMessageException e) {
					System.err.println("No message resource found for " + value
		...
		return new Converter<ClubMember, String>() {
			public String convert(ClubMember clubMember) {
				if (clubMember.getLastName() != null
						&& !StringUtils.isEmpty(clubMember.getLastName())
						&& clubMember.getFirstName() != null
						&& !StringUtils.isEmpty(clubMember.getFirstName()))
					return clubMember.getLastName() + ", "
							+ clubMember.getFirstName();
				else
					return new StringBuilder().append(clubMember.getLastName())
							.append(' ').append(clubMember.getFirstName())
							.append(' ').append(clubMember.getAddress())
							.append(' ').toString();
			}
		};
	}
		...
		return new Converter<Person, String>() {
			public String convert(Person person) {
				if (person.getLastName() != null
					...
					return new StringBuilder().append(person.getLastName())
							.append(' ').append(person.getFirstName())
							.append(' ').append(person.getAddress())
							.append(' ').toString();
			}
		};
	}
	...
	public Converter<UserPermission, String> getUserPermissionToStringConverter() {
		return new Converter<UserPermission, String>() {
			...
			public String convert(UserPermission value) {
				String output = value.toString();
				try {
					output = messageSource.getMessage(value.getName(), null,
							LocaleContextHolder.getLocale());
				} catch (NoSuchMessageException e) {
					System.err.println("No message resource found for " + value
							+ " add this to the resource bundle");
